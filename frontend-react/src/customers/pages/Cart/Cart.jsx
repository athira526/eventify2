...
import { createBooking } from "../../../State/Customers/Bookings/Action";
import { findCart } from "../../../State/Customers/Cart/cart.action";
import { isValid } from "../../util/ValidToBooking";
...
const Cart = () => {
  ...
  const handleSubmit = (values, { resetForm }) => {
    const data = {
      ...
      booking: {
        festId: cart.cartItems[0].activity?.fest.id,
        location: {
  ...
  const createBookingUsingSelectedAddress = (location) => {
    const data = {
  ...
  return (
    <Fragment>
      {cart.cartItems.length > 0 ? (
          ...
          <section className="lg:w-[70%] flex justify-center px-5 pb-10 lg:pb-0">
            ...
            <div className="flex gap-5 flex-wrap justify-center">
              {auth.user?.addresses.map((item, index) => (
                <AddressCard
                  handleSelectAddress={createBookingUsingSelectedAddress}
                  item={item}
                  showButton={true}
                />
              ))}
            ...
            {/* <div className="flex justify-center items-center h-[90vh]">
              <Card className="billDetails px-5 text-sm w-[20vw] p-10 space-y-5">
                ...
                <Button
                  onClick={createBookingUsingSelectedAddress}
                  variant="contained"
                >
                  Checkout
                </Button>
        ...
        <div className="flex h-[90vh] justify-center items-center">
          <div className="text-center space-y-5">
            <ConfirmationNumberIcon sx={{ width: "10rem", height: "10rem" }} />
            <p className="font-bold text-3xl">No Events added yet!</p>
          </div>
        </div>
      )}
      ...
      <Snackbar
        ...
        message="Please Add Items Only From One Fests At time"
      />
    </Fragment>
  );
};
