...
public class CartServiceImplementation implements CartSerive {
	...
	@Autowired
	private activityRepository menuItemRepository;

	@Override
	public CartItem addItemToCart(AddCartItemRequest req, String jwt) throws UserException, ActivityException, CartException, CartItemException {

		User user = userService.findUserProfileByJwt(jwt);
		
		Optional<Activity> menuItem=menuItemRepository.findById(req.getMenuItemId());
		if(menuItem.isEmpty()) {
			throw new ActivityException("Menu Item not exist with id "+req.getMenuItemId());
		}

		Cart cart = findCartByUserId(user.getId());

		for (CartItem cartItem : cart.getItems()) {
			if (cartItem.getActivity().equals(menuItem.get())) {
		...
		newCartItem.setActivity(menuItem.get());
		newCartItem.setQuantity(req.getQuantity());
		newCartItem.setCart(cart);
		newCartItem.setSkills(req.getSkills());
	...
	public CartItem updateCartItemQuantity(Long cartItemId,int quantity) throws CartItemException {
		...
		cartItem.get().setTotalPrice((cartItem.get().getActivity().getPrice()*quantity));
	...
	public Long calculateCartTotals(Cart cart) throws UserException {
		...
		Long total = 0L;
		for (CartItem cartItem : cart.getItems()) {
			total += cartItem.getActivity().getPrice() * cartItem.getQuantity();
		}
		return total;
	}
